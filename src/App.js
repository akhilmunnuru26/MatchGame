...
const TabItem = props => {
  ...
  const changeColor = isActive ? 'active-tab' : ''

  return (
    <li className="tab-item">
      <button
        onClick={onClickButton}
        className={`tab-button ${changeColor}`}
        type="button"
      >
        {displayText}
      </button>
...
class App extends Component {
  ...
  onClickImage = id => {
    const {matchImageDetails} = this.state
    const index = Math.ceil(Math.random() * imagesList.length)
    this.setState({matchImageDetails: imagesList[index]})
    const isImageMatched = matchImageDetails.id === id
    if (isImageMatched) {
  ...
  decrementSeconds = () => {
    const {counter, isGameInProgress} = this.state
    const isTimeLimitFinished = counter === 0

    if (isTimeLimitFinished) {
      this.clearTimeInterval()
  ...
  render() {
    ...
    const imgUrl = matchImageDetails.imageUrl
      ...
      <div className="bg-container">
        <nav className="nav-bar">
          <ul className="nav-items-container">
            ...
            <li className="score-timer-nav-box">
              <div className="score-timer-box">
                ...
                <div className="timer-box">
                  ...
                  />
                  <p className="time">{counter} sec</p>
                </div>
              </div>
            </li>
          </ul>
